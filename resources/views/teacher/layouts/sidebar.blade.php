      ...
      <div class="user-panel mt-3 pb-3 mb-3 d-flex">
        <div class="image">
          <img class="profile-user-img img-fluid img-circle"
                       src="{{(!empty(Auth::user()->image))?url('upload/teacherimage/'.Auth::user()->image):url('upload/userimage.png')}}"
                       alt="User profile picture" style="width: 50px;height: 50px">
        </div>
        <div class="info">
          <a href="{{ route('teacher.profiles.view') }}" class="d-block">{{ Auth::user()->name}}</a>
        </div>
      </div>
      ...
      <nav class="mt-2">
        ...
        {{--   <li class="nav-item {{ request()->is('admin/user-list') ? 'menu-open' : '' }}">
            ...
            <ul class="nav nav-treeview">
              ...
              <li class="nav-item">
            <a href="{{ route('teacher.student.view') }}" class="nav-link {{ request()->is('student-view') ? 'active' : '' }}">
